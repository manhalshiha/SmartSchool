...
namespace SmartSchool.Web.Services
{
    ...
    {
        protected override TimeSpan RevalidationInterval =>/* throw new NotImplementedException();*/TimeSpan.FromMinutes(20);

        protected override async Task<bool> ValidateAuthenticationStateAsync(AuthenticationState authenticationState, CancellationToken cancellationToken)
        {
            await using var scope = serviceScopeFactory.CreateAsyncScope();
            var userManger = scope.ServiceProvider.GetRequiredService<UserManager < IdentityUser >> ();
            return await ValidateSecurityTimeStampAsync(userManger, authenticationState.User);
        }
